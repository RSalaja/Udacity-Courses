...
# such as a list. Many of the built-in functions we’ve used so far, like 'enumerate,'
# return an iterator.
#
...
#
# Remember, since this returns an iterator, we can convert it to a list or iterate
# through it in a loop to view its contents. For example, this code:
#
# for x in my_range(5):
#     print(x)
# outputs:
...
# Here’s an excerpt from a stack overflow page that addresses this:
#
# Generators are a lazy way to build iterables.
# They are useful when the fully realized list would not fit in memory,
# or when the cost to calculate each list element is high and you want to
# do it as late as possible. But they can only be iterated over once
