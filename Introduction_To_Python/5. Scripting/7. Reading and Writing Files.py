# f = open(r'C:\Users\salaj\some_file.txt', 'r')
# file_data = f.read()
# f.close()
# print(file_data)

# First open the file using the built-in function, open.
...
# Python interacts with the file itself. Here, we assign this object to the variable f.
# There are optional parameters you can specify in the open function. One is the mode
# in which we open the file. Here, we use r or read only. This is actually the default value for the mode argument.
...
# If you open an existing file in writing mode, any content
# that it had contained previously will be deleted.
# If you're interested in adding to an existing file,
# without deleting its content, you should use the append ('a') mode instead of write.
